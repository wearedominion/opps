...
- **Progression labeling (code issue — mostly resolved).** Rank titles are layered on the Clout-driven level. They were one-per-level, then **bands** of 10; since DOM-124 `data/ranks.json` holds the handoff's **100 titles** (NOBODY → STREET DON), **spread** across the 120-level cap so each lasts one or two levels, and `levelsPerRank` is retired. Still open: whether Rank titles stay as flavour at all, and surfacing Clout itself (the HUD now shows it as the hero figure). The prototype surfaces progression as a **Rank title** under the HUD label "RANK", plus a numeric level — while **Clout** (our canonical progression stat) isn't shown anywhere. Canonical naming defines **Clout → level** and never mentions Rank. Decide: keep Rank titles as flavor layered on the Clout-driven level (and surface Clout somewhere), or drop them. Touches `hud.js` / `stats.js` / `ui.js` / `index.html` ("RANK") and `data/ranks.json`.
