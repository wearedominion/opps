...
### 9.5 `gear.json` — the gear catalog (DOM-73, ratified 2026-09-11)
...
| 4 | Profile gear tab | **Real catalog.** The prototype body-doll placeholder (`GEAR_SLOTS`/`GEAR_ITEMS`) is deleted; the loadout runs on the catalog's types. ~~Closes the profile TDD §7 open decision: gear lives in `store.json`, no separate `gear.json`.~~ **Superseded 2026-09-14 (DOM-123): the catalog was renamed to `gear.json` and owns every item definition; `store.json` is gone.** |
...
### 9.11 Plug quests — DOM-90 decision record (ratified 2026-09-14, Jake)

**Quests must not obsolete the Moves ladder.** Four decisions, all ratified 2026-09-14:

| # | Question | Decision |
|---|----------|----------|
| 1 | Reward pricing | **Bonus-on-top rule.** Quest steps route through the EXISTING faucets — required jobs and fights pay their normal rates while ground, so the 60:35 Clout mix is untouched by construction. Completion pays a one-time bonus = `hours` of best-job income at the quest's gate (cash + the same hours' best-job Clout), generated by gen-catalog like every other catalog. |
| 2 | Step types (v1) | **`job` (N completions) · `fight` (N wins) · `item` (own it, checked live)**. Pay-cash steps deferred. |
| 3 | Repeatability | **One-shot per player.** The catalog is a finite faucet; the sim reports the totals (932 lifetime Clout ≈ 0.001% of the curve). Repeatables need a mix re-solve and their own ticket. |
| 4 | Gear rewards | **Allowed.** One own-once item per quest (skipped if owned), granted with `src: 'quest'`, auto-fielded when a slot is open. |

**Shapes:**

- `data/quests.json` (generated): `{ id, plug, name, desc, levelReq, hours, steps: [{type, id, count?}], reward: {cash, clout, item?} }`. The generator refuses a step or reward id that doesn't resolve in the same write, and a step gated above its quest's own level.
...
- `gear.json` items gain `rarity: 'grey'|'green'|'blue'|'purple'|'orange'` (authored in
